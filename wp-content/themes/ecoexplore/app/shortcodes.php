    ...
    <section class="form-section fieldset" data-section-number="1" aria-hidden="false">
      <div class="form-row">
        ...
        <div class="dz-wrapper">
          <?php echo do_shortcode('[wp-dropzone id="dropfile" max-file-size="10" accepted-files="image/*" max-files="1" resize-width="1200" resize-height="1200" resize-quality="0.8" resize-method="contain" title="Drop image here or click to upload" callback="success: function(file, response) { jQuery(\'#btn-first-next-step\').removeClass(\'disabled\'); jQuery(\'#dz-files\').val(response.data); }"]'); ?>
          <?php //echo do_shortcode('[wp-dropzone id="dropfile" max-file-size="10" accepted-files="image/*" max-files="1" title="Drop image here or click to upload" callback="error: function(file, errorMsg) {console.log(errorMsg); }"]'); ?>
